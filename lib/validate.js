 ...
 * @param {Object} options.imageChecksum - image checksums
 * @param {String} options.imageChecksum.crc32 - CRC32
 * @param {String[]} options.checksumAlgorithms - algorithms to calculate checksums for
...
exports.usingChecksum = (deviceFileDescriptor, options = {}) => {
  let checksumAlgorithms = options.checksumAlgorithms || [];

  if (!checksumAlgorithms && options.imageChecksum) {
    checksumAlgorithms = _.union(checksumAlgorithms, Object.keys(options.imageChecksum));
  }
  ...
  return checksum.calculateDeviceChecksum(deviceFileDescriptor, {
    ...
    if (options.imageChecksum) {
      Object.keys(options.imageChecksum).forEach((algorithm) => {
        if (results[algorithm] && options.imageChecksum[algorithm] !== results[algorithm]) {
          throw errors.ValidationError;
        }
      });
    }
