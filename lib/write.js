...
var bmapflash = require('bmapflash');
  ...
  win32.prepare(drive.device).then(function() {
    ...
    if (options.bmap) {
      return new Promise(function(resolve, reject) {
        const flasher = bmapflash.flashImageToFileDescriptor(image.stream, drive.fd, options.bmap);
    ...
    if (!options.check) {
      return win32.rescan().then(function() {
        return fs.closeAsync(drive.fd).then(() => {
          emitter.emit('done', {
    ...
    if (options.bmap) {
      return new Promise(function(resolve, reject) {
        const validator = bmapflash.validateFlashedImage(drive.fd, options.bmap);
      ...
      }).tap(win32.rescan).then((invalidRanges) => {
        emitter.emit('done', {
    ...
    return new Promise(function(resolve, reject) {
      var checksumStream = new CRC32Stream();
      var deviceStream = fs.createReadStream(null, {
        fd: drive.fd,
        start: 0
      });
