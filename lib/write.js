...
exports.write = function(drive, image, options) {
  ...
  denymount(devicePath, function(callback) {
    ...
    return win32.prepare(devicePath).then(function() {
      ...
      if (options.bmap) {
        return fs.openAsync(devicePath, 'rs+').then((fileDescriptor) => {
          return new Promise(function(resolve, reject) {
            const flasher = bmapflash.flashImageToFileDescriptor(image.stream, fileDescriptor, options.bmap);
      ...
      if (options.bmap) {
        return fs.openAsync(devicePath, 'rs+').then((fileDescriptor) => {
          return new Promise(function(resolve, reject) {
            const validator = bmapflash.validateFlashedImage(fileDescriptor, options.bmap);
